...
namespace Tools
{
	public class OperationResult<T> : IOperationResult<T> where T : class
	{
		...
		public static OperationResult<T> BadResultFormat(string message, object arg0)
		{
			return new OperationResult<T> { Result = false, Message = string.Format(message, arg0) };
		}
		#endregion
